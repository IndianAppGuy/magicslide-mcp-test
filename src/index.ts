...
interface DetailsAPIResponse {
    ...
    model?: string;
    template?: string;
    imageForEachSlide?: boolean;
    image_source?: string;
    msSummaryText?: string;
}

interface MagicSlidesCreateResponse {
    success: boolean;
...
async function fetchYoutubeTranscript(ytUrl: string): Promise<string> {
    try {
        ...
        throw new Error(
            "Unknown error occurred while fetching YouTube transcript."
        );
    }
}
...
async function fetchAccountInfo(
    ...
    try {
        ...
        if (
            ...
            throw new Error(
                "Invalid access data received. Please check your access ID."
            );
        }
...
async function fetchDetailsFromAPI(
    ...
    try {
        ...
        if (data.slideCount && userParams.slideCount === undefined) {
            const parsed =
                typeof data.slideCount === "string"
                    ? parseInt(data.slideCount, 10)
                    : data.slideCount;
            if (!Number.isNaN(parsed)) slideCount = parsed as number;
        }

        if (data.language) language = data.language;
        if (data.model && !userParams.model) model = data.model.toLowerCase();
        if (data.template && !userParams.template)
            template = data.template.toLowerCase();
        if (
            typeof data.imageForEachSlide === "boolean" &&
            userParams.imageForEachSlide === undefined
        ) {
...
async function createPPTFromText(
    userText: string,
    accessId: string
): Promise<{
    success: boolean;
...
type CreatePPTArgs = { userText: string; accessId?: string };
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    accessId: {
                        type: "string",
                        description:
                            "Optional MagicSlides access ID override. Defaults to the MAGICSLIDES_ACCESS_ID environment variable configured via Claude Desktop.",
                    },
                },
                required: ["userText"],
            },
...
server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
    if (request.params.name === "create_ppt_from_text") {
        try {
            ...
            if (!resolvedAccessId) {
                throw new Error(
                    "Missing MagicSlides access ID. Provide 'accessId' in the tool arguments or set the MAGICSLIDES_ACCESS_ID environment variable in your Claude Desktop config."
                );
            }
...
Copy and paste this URL into your browser to open your presentation in the MagicSlides editor.`,
        ...
        try {
            const args = request.params.arguments as { ytUrl: string };
            if (!args || !args.ytUrl) throw new Error("Missing YouTube URL parameter.");
